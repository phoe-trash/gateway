...
;;        (connection-4 (make-instance 'standard-connection :port n-port :type :client))
;;        (connections (list connection-1 connection-2 connection-3 connection-4))
;;        (data-1 '(open-gateway))
;;        (data-2 '(open-gateway "foo"))
;;        (data-3 '(open-gateway "bar" 123 "baz" "quux"))
;;        (data-4 '(garbage input)))
;;     (send connection-1 data-1)
;;     (send connection-2 data-2)
;;     (send connection-3 data-3)
;;     (send connection-4 data-4)
;;     (loop do (sleep 0.01) until (= 3 (length (e-connections crown))))
;;     (assert (data-equal (receive connection-1) `(ok ,data-1)))
;;     (assert (data-equal (receive connection-2) `(ok ,data-2)))
;;     (assert (data-equal (receive connection-3) `(ok ,data-3)))
;;     (assert (data-equal (receive connection-4) `(error unknown-function ,data-4)))
